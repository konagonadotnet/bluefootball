<?php
    ...
    class JleageD1MatchResults2013Table extends Table {

        public function initialize(array $config) {
            $this->setTable('jleage_d1_match_results2013');
        ...
        /*
         * id、TeamName、 ShortTeamName取得メソッド
         */
        public function getTeamNameData() {
            // Select文実行
            $query_data = $this->find()
            ->select([
                'id', // チームID
                'TeamName', // チーム名
                'ShortTeamName', // チーム名の略称
            ])
            ->all();
            if($query_data->count() == 0) {
                // データが取得できなかった場合
         ...
         * 全チーム名データ登録メソッド
         */
        public function insertTeamNameData($data) {
            // Entityオブジェクトに変換
